...
# --- 3b. NEW: Indicator filter (variables dropdown) ---
st.sidebar.subheader("Indicators")
all_indicators = sorted(filtered_nodes_df["node"].unique().tolist())
selected_indicators = st.sidebar.multiselect(
    "Select indicators (variables):",
    options=all_indicators,
    default=all_indicators  # default: show all variables
)

if selected_indicators:
    # Keep edges where either endpoint is one of the selected indicators
    filtered_edges_df = filtered_edges_df[
        filtered_edges_df["variable1"].isin(selected_indicators)
        | filtered_edges_df["variable2"].isin(selected_indicators)
    ].copy()

    if filtered_edges_df.empty:
        st.warning(
            f"⚠️ No edges meet criteria after indicator filter. "
            f"Selected indicators: {selected_indicators}"
        )
        st.stop()

    # Rebuild node list from the remaining edges
    nodes_from = filtered_edges_df[["variable1", "category1"]].rename(
        columns={"variable1": "node", "category1": "category"}
    )
    nodes_to = filtered_edges_df[["variable2", "category2"]].rename(
...
st.sidebar.write(f"**Nodes:** {len(filtered_nodes_df)}")
st.sidebar.write(f"**Categories:** {len(unique_categories)}")
st.sidebar.caption("Filtered for p < 0.05, correlation threshold, categories, and indicators")
